...
def display_results(top_outflows: pd.Series, top_inflows: pd.Series):
    """結果を表示"""
    print("=" * 60)
    print(f"全期間での支出上位{TOP_N}つの適用")
    print("=" * 60)
    for i, (desc, amount) in enumerate(top_outflows.items(), 1):
        print(f"{i}. {desc:30s} ¥{int(amount):,}")
    
    print("\n" + "=" * 60)
    print(f"全期間での収入上位{TOP_N}つの適用")
    print("=" * 60)
...
def main():
    ...
    # 結果表示
    display_results(top_outflows, top_inflows)
