# -*- coding: utf-8 -*-
"""秀夫統合口座 主な支出・収入グラフ生成スクリプト（日本語フォント対応）"""
...
def main():
    ...
    print(f"\n支出上位{TOP_N}項目（全期間）:")
    for desc, amount in top_outflows.items():
        print(f"  {desc}: ¥{amount:,.0f}")
    
    print(f"\n収入上位{TOP_N}項目（全期間）:")
    for desc, amount in top_inflows.items():
        print(f"  {desc}: ¥{amount:,.0f}")
    ...
    # 保存
    out_path = OUTPUT_DIR / "hideo_chart_top_flows_total.pdf"
