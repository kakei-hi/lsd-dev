# -*- coding: utf-8 -*-
"""秀夫統合口座残高推移グラフ生成スクリプト（日本語フォント対応・ゼロ到達予測付き）"""
...
def plot_balance_chart(daily_bal: pd.DataFrame, future_idx: pd.Index, 
    ...
    # グラフを2027年までに制限
    date_limit = pd.to_datetime("2026-12-31")
    
    # 実績残高（2030年までのデータのみ）
    daily_filtered = daily_bal[daily_bal["Date"] <= date_limit]
    ax.plot(daily_filtered["Date"], daily_filtered["Balance"], 
            color="#1f77b4", lw=2.0, label="残高（実績）", marker="o", markersize=3, alpha=0.7)
    
    # 予測直線（2030年までのデータのみ）
    if slope < 0:
        future_filtered = future_idx[future_idx <= date_limit]
        y_future_filtered = y_future[:len(future_filtered)]
        if len(future_filtered) > 0:
            ax.plot(future_filtered, y_future_filtered, color="red", ls="--", 
                    lw=2.0, label="トレンド予測")
    ...
    # グラフの装飾
    ax.set_title("秀夫統合口座残高の推移", fontsize=TITLE_FONTSIZE, fontfamily=JP_FONT)
    ...
    # X軸の上限を2030年12月31日に設定
    ax.set_xlim(left=daily_bal["Date"].min(), right=date_limit)    
    # Y軸の範囲を設定
    ax.set_ylim(5000000, 8000000)    
    # ゼロ到達予測日をテキストで表示
    if zero_date is not None:
        textstr = f"ゼロ到達予測日: {zero_date}"
...
def main():
    """メイン処理"""
    # データ読込と前処理
    csv_path = BASE_DIR / "Hideo_balance.csv"
    df = load_and_prepare_data(csv_path)
    daily_bal = create_daily_balance(df)
    
    print(f"データ読込完了: {csv_path}")
    print(f"データ期間: {daily_bal['Date'].min().date()} ～ {daily_bal['Date'].max().date()}")
    print(f"初期残高: ¥{daily_bal['Balance'].iloc[0]:,.0f}")
    print(f"最終残高: ¥{daily_bal['Balance'].iloc[-1]:,.0f}")
    
    # 回帰分析
    recent, slope, intercept = perform_linear_regression(daily_bal)
    print(f"\n回帰分析結果（直近{REGRESSION_DAYS}日間）:")
    print(f"  サンプル数: {len(recent)}")
    print(f"  傾き（日次変化率）: ¥{slope:.2f}/日")
    ...
    if zero_date is not None:
        print(f"  ゼロ到達予測日: {zero_date}")
    else:
        print(f"  ゼロ到達予測: なし（残高は増加傾向）")
